...
class SaleOrderLine(models.Model):
    ...
    crm_line_id = fields.Many2one('crm.service.line')

    @api.constrains('product_id' , 'source_language','target_language','estimated_number_of_words' , 'estimated_number_of_pages' , 'price_unit','tax_id' ,'discount')
    def change_values(self):
        for rec in self:
            if rec.crm_line_id:
                rec.crm_line_id.write({
                    'service_id' : rec.product_id.id,
                    'pages' :rec.estimated_number_of_pages,
                    'words': rec.estimated_number_of_words,
                    'source_lang':rec.source_language.id,
                    'target_lang':rec.target_language.id,
                    'Rate': rec.price_unit,
                    'vat': rec.tax_id.id,
                    'disc': rec.discount
                })
