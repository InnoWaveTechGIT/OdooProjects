...
class Meals(http.Controller):
    ...
    def get_events_for_class(self,**kw):
        user_id=request.env.uid
        result = []
        user_id = request.env['res.users'].sudo().search([('id' , '=' , user_id)])
        if user_id.user_type == 'Teacher':
            class_id = request.env['class.school.app'].sudo().search([('teacher_id' , '=' , user_id.id)] , limit=1) 
            event_id = request.env['events.school.app'].sudo().search(['|' , ('class_id' , '=' , class_id.id) , ('event_type' , '=' , 'Public')] ) 
            if event_id:
            ...
            class_id = request.env['class.school.app'].sudo().search([('student_ids' , '=' , user_id.id)] , limit=1) 
            event_id = request.env['events.school.app'].sudo().search(['|' , ('class_id' , '=' , class_id.id) , ('event_type' , '=' , 'Public')]) 
            if event_id:
                for event in event_id:
                    result.append({
