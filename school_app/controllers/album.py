...
class Album(http.Controller):
    ...
    def get_student_albums(self,student_id = None ,page=None,limit=None,**kw):
        ...
        if user_id.user_type == 'Student':
            ...
            albums = request.env['album.school.app'].sudo().search(['|',('status' , '=' , 'Public') , ('student_ids2' , 'in' ,student_id )], limit=limit, offset=(page - 1) * limit)
            album_obj_count = request.env['album.school.app'].sudo().search_count(['|',('status' , '=', 'Public') , ('student_ids2' , 'in' , student_id)])
            if len(albums) != 0:
        ...
        if user_id.user_type == 'Teacher':
            class_id = request.env['section.school.app'].sudo().search([('teacher_id' , '=' , user_id.id)])
            albums = request.env['album.school.app'].sudo().search(['|',('status' , '=' , 'Public') , ('class_id' , '=' ,class_id.id )], limit=limit, offset=(page - 1) * limit)
            album_obj_count = request.env['album.school.app'].sudo().search_count(['|',('status' , '=', 'Public') , ('student_ids2' , 'in' , student_id)])
            if len(albums) != 0:
                totalpages = math.ceil(album_obj_count / limit)
            else:
    ...
    def get_student_albums_by_id(self,album_id,page=None,limit=None,**kw):
        user_id=request.env.uid
        if limit :
            limit = int (limit)
        ...
        result = []
        user_id = request.env['res.users'].sudo().search([('id' , '=' , user_id)])
        albums = request.env['album.school.app'].sudo().search([('id' , '=' , album_id) ], limit=limit, offset=(page - 1) * limit)
        album_obj_count = request.env['album.school.app'].sudo().search_count([('id' , '=' , album_id)])
        if len(albums) != 0:
                totalpages = math.ceil(album_obj_count / limit)
        else:
            if album_obj_count and int(limit)>album_obj_count:
    ...
    def get_student_images(self , student_id ,page=None,limit=None,**kw):
        user_id=request.env.uid
        if limit :
            limit = int (limit)
        ...
        if user_id.user_type == 'Student':
            student_id = int(student_id)
            albums = request.env['images.school.app'].sudo().search(['|',('status' , '=' , 'Public') , ('student_ids2' , 'in' ,student_id )], limit=limit, offset=(page - 1) * limit)
            album_obj_count = request.env['images.school.app'].sudo().search_count(['|',('status' , '=' , 'Public') , ('student_ids2' , 'in' ,student_id )])
            if len(albums) != 0:
                totalpages = math.ceil(album_obj_count / limit)
            else:
                if album_obj_count and int(limit)>album_obj_count:
        ...
        if user_id.user_type == 'Teacher':
            class_id = request.env['section.school.app'].sudo().search([('teacher_id' , '=' , user_id.id)])
            albums = request.env['images.school.app'].sudo().search(['|',('status' , '=' , 'Public') , ('class_id' , '=' ,class_id.id )], limit=limit, offset=(page - 1) * limit)
            album_obj_count = request.env['images.school.app'].sudo().search_count(['|',('status' , '=' , 'Public') , ('class_id' , '=' ,class_id.id )])
            if len(albums) != 0:
                totalpages = math.ceil(album_obj_count / limit)
            else:
                if album_obj_count and int(limit)>album_obj_count:
