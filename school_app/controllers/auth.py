...
class Auth(http.Controller): 
    ...
    def log_in_school(self,idd= None, **kw):               
            ...
            if not device_id:
                response=json.dumps({"data":[],'message': 'Device ID is required'})
            ...
            if len(user_id)==0 :
                ...
                is_ther_device_id = request.env['student.devices'].search([('user_id' , '=' , user_id.id)])
                if is_ther_device_id :
                    try:
                        check = False
                        for i in is_ther_device_id.device_ids:
                            if device_id == i.identification:
                                check=True
                        if not check:
                            if len(is_ther_device_id.device_ids) == 0 :
                                request.env['student.devices.line'].create({
                                    'user_id' : is_ther_device_id.id,
                    ...
                    is_ther_device_id = request.env['student.devices'].create({
                            'user_id' : user_id.id,
                ...
                if user_id.user_type == 'Student':
                    for i in user_id.student_ids:

                        chiled.append({
                            'id' : i.id , 
                            'name' : i.name,
                            'section_id' : i.section_id.name,
                            'code' : i.code ,
                            'image' : i.image_url
                        })
                if len(is_ther_token) != 0 :
                    ...
                    user_details = [{"id":user_id.id,"name":user_id.name,"phone":user_id.login,'image_path':user_id.image_url, 'type' : user_id.user_type , 'chiled' : chiled}]
