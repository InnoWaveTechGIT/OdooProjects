...
class Data(http.Controller):
    ...
    def get_student_data(self,**kw):
        ...
        if user_id.user_type == 'Teacher':
            ...
            for i in class_id.student_ids2:
                result.append({
                    'id' : i.id,
                    'name' : i.name,
                    'image' : i.image_url,
    ...
    def get_numberofunread(self, **kw):
        ...
        if user.user_type == 'Teacher':
            class_id = http.request.env['section.school.app'].sudo().search([('teacher_id', '=', user_id)])
        ...
        if user.user_type == 'Student':
            class_id = http.request.env['section.school.app'].sudo().search([('student_ids', 'in', user_id)])
            albums = http.request.env['album.school.app'].sudo().search_count([
                ('user_ids', 'not in', (user_id,)), '|', ('status', '=', 'Public'), ('student_ids', 'in', (user_id,))
            ])
            student_homework = http.request.env['home.work.lines'].sudo().search_count([
    ...
    @http.route('/student/parent',  auth="api_key",csrf=False, website=True, methods=['GET'])
    def get_student_parent(self, student_id):
        user_id=request.env.uid
        result = []
        user_id = request.env['res.users'].sudo().search([('id' , '=' , user_id)])
        if user_id.user_type != 'Student':
            student_obj = request.env['students.school.app'].sudo().search([('id' , '=' , int(student_id))] , limit=1) 
            for i in student_obj:
                result.append({
                    'id' : i.id,
                    'parent' : {
                        'id': i.parent_id.id,
                        'name': i.parent_id.name or ''
                    }
                })
            response = json.dumps({'data': result , 'message' : 'All Data' , 'code' : 200})
            return Response(
    ...
    @http.route('/student/teacher',  auth="api_key",csrf=False, website=True, methods=['GET'])
        ...
        if user_id.user_type == 'Student':
            ...
            for i in student_obj:
                result.append({
                    'id' : i.id,
                    'main_teacher' : {
                        'id': i.section_id.teacher_id.id,
                        'name': i.section_id.teacher_id.name or ''
                    },
                    'teachers' : [{
                        'id': teacher.id,
                        'name': teacher.name or ''
                    } for teacher in i.section_id.teacher_ids]
                })
            response = json.dumps({'data': result , 'message' : 'All Data' , 'code' : 200})
            return Response(
                response, status=200,
