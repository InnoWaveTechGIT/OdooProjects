...
class ContractController(BaseController):
    ...
    def CreateContract(self, **kwargs):       
        ...
        if not rquested_user_id or not provided_user_id or not contract_id or not transaction_amount or not commission_amount or not tax_percentage or not description or not company_id or not service_id:
            ...
            error_message = f"rquested_user_id, provided_user_id, contract_id, transaction_amount, commission_amount, tax_percentage, description, company_id and service_id are required."
        ...
        #                                         'description': "contract_deposite_transaction",
        #                                         'withdraw_request': transaction_amount,
        #                                         'issued': 0,
        #                                         'used': 0,                                                
        #                                         'order_model': 'account.payment',
        #                                         'order_id': provided_payment.id,                                                        
        #                                     })
        return {'status': 200, 
            ...
            if action_type == 1:
                if not rquested_payment or not history_payment:
                    ...
                    if rquested_payment.state in ['draft']:
                        history_payment.with_user(self.get_user_id()).sudo().write({'withdraw_request': 0.0, 'used': rquested_payment.amount}) 
                ...
                # if rquested_payment.state in ['draft','in_process']:
                if rquested_payment.state in ['draft']:
                    history_payment.with_user(self.get_user_id()).sudo().write({'withdraw_request': 0.0}) 
