...
class ReportSaleDetails(models.AbstractModel):
    ...
    def get_sale_details(self, date_start=False, date_stop=False, config_ids=False, session_ids=False):
                    ...
                    if not payment['cash']:
                        ...
                        for cash_move in cash_moves:
                            ...
                            if cash_move.move_id.journal_id.id == payment['journal_id']:
                                ...
                                print("lissssssssst ", cash_in_out_list)
            ...
            if not is_cash_method:
                ...
                for cash_move in cash_moves:
                    if cash_move.amount > 0:
                        ...
                        cash_out_count += 1
                        name = f'Cash out {cash_out_count}'
                    if cash_move.move_id.journal_id.id :
                        cash_in_out_list.append({
                            'name': cash_move.payment_ref if cash_move.payment_ref else name,
                            'amount': cash_move.amount
                        })
                print("cash move list ", cash_in_out_list)
                payments.insert(0, {
                    'name': cash_name,
                    'total': 0,
        ...
        print("paaaaaaaaaaaaay ", payments,orders)
        print("product info ", products_info)
        print("product  ", products)
        _logger.info('----------------------- payment ', payments)
        return {
