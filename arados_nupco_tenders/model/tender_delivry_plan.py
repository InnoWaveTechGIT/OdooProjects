...
class TenderlinesDetails(models.Model):
    ...
    lot_value = fields.Monetary(string='Lot Value', compute='_compute_lot_value', store=True, currency_field='currency_id')

    currency_id = fields.Many2one('res.currency', string='Currency', related='sale_line_id.order_id.currency_id', store=True)
    start_date = fields.Date()
    last_date = fields.Date()
    so_order_date = fields.Date()
    delivered_value = fields.Float(related='sale_line_id.qty_delivered')
    un_delivered_value = fields.Float( related='sale_line_id.qty_to_deliver')
    adjust_value = fields.Float()
    late_delivery_sub = fields.Char()
    late_delivery_amg = fields.Char('Late Delivery (company)')
    status = fields.Char()
    discount = fields.Float(related='sale_line_id.discount')
    ...
    @api.depends('sale_line_id.price_subtotal')
    def _compute_lot_value(self):
        for record in self:
            record.lot_value = record.sale_line_id.price_subtotal
